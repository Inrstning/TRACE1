...
class GraSHSearchJob(AutoSearchJob):
    ...
    def run(self):
        ...
        # get configspace object corresponding to given config
...
class GraSHWorker(Worker):
    ...
    responsible for creating train jobs for the hyperparameter configurations
    ...
    def _compute(self, config_id, config, budget):
        """
        Creates a train job for a given hyperparameter configuration and returns
    ...
    def _determine_epochs(self, config, budget):
        """
        Determines the training epochs for a given configuration and trial budget
        """
